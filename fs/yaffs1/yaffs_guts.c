...
yaffs_off_t yaffs_GetFileSize(yaffs_Object *obj);
...
static int yaffs_CheckChunkErased(struct yaffs_DeviceStruct *dev,int chunkInNAND)
{
	...
	printk("printk Check init %d &init %8.8x cmpbuf %8.8x data %8.8x spare %8.8x\n",&init,init,cmpbuf,data,spare);
...
static int yaffs_WriteNewChunkToNAND(struct yaffs_DeviceStruct *dev, const __u8 *data, yaffs_Spare *spare,int useReserve)
{
	...
	do{
		printk("Call yaffs_AllocateChunk()\n");
		...
		if(chunk >= 0)
		{
			...
			printk("Call yaffs_CheckChunkErased()\n");
			// writeOk = yaffs_CheckChunkErased(dev,chunk);
#endif		
			if(!writeOk)
			{
				printk(("**>> yaffs chunk %d was not erased" TENDSTR),chunk);
				T(YAFFS_TRACE_ERROR,(TSTR("**>> yaffs chunk %d was not erased" TENDSTR),chunk));
			}
			else
			{
				printk("Call yaffs_WriteChunkToNAND()\n");
			...
			if(writeOk)
			{
				...
				//Todo
				printk("Call yaffs_HandleWriteChunkOk()\n");
				yaffs_HandleWriteChunkOk(dev,chunk,data,spare);
			}
			else
			{
				printk("Call yaffs_HandleWriteChunkError()\n");
				yaffs_HandleWriteChunkError(dev,chunk);
			}
		}
		printk("chunk %d writeOk? %d\n",chunk,writeOk);
								 ...
								 __u32 rdev)
{
	...
	printk("\tCall yaffs_FindObjectByName\n");
	// Check if the entry exists. If it does then fail the call since we don't want a dup.
	if(yaffs_FindObjectByName(parent,name))
	{
		return NULL;
	}
	printk("\tCall yaffs_CreateNewObject()\n");
	in = yaffs_CreateNewObject(dev,-1,type);
	printk("\tAft Call yaffs_CreateNewObject()\n");
	if(in)
	{
		in->chunkId = -1;
		in->valid = 1;
		...
		// puts("\tCall yaffs_SetObjectName()\n");
		yaffs_SetObjectName(in,name);
		in->dirty = 1;
		
		// printk("\tCall yaffs_AddObjectToDirectory()\n");
		...
		printk("\tCall yaffs_UpdateObjectHeader()\n");
		if(/*yaffs_GetNumberOfFreeChunks(dev) <= 0 || */
		   yaffs_UpdateObjectHeader(in,name,0) < 0)
		{

			printk("\tCall yaffs_AbortHalfCreatedObject()\n");
...
static int yaffs_WriteNewChunkWithTagsToNAND(yaffs_Device *dev, const __u8 *buffer, yaffs_Tags *tags, int useReserve)
{
	...
	printk("Ret yaffs_SpareInitialise()\n");
	yaffs_SpareInitialise(&spare);
	...
	printk("Ret yaffs_WriteNewChunkToNAND()\n");
...
int yaffs_UpdateObjectHeader(yaffs_Object *in,const char *name, int force)
{
    ...
    printk("\t if(!in->fake || force)\n");
    if(!in->fake || force)
    {
  		printk("\tCall yaffs_CheckGarbageCollection()\n");
		yaffs_CheckGarbageCollection(dev);		
		...
		yaffs_CalcTagsECC(&newTags);
 		...
 		printk("Call yaffs_WriteNewChunkWithTagsToNAND()\n");
		// Create new chunk in NAND
		newChunkId = yaffs_WriteNewChunkWithTagsToNAND(dev,bufferNew,&newTags, (prevChunkId >= 0) ? 1 : 0 );
    	printk("Aft yaffs_WriteNewChunkWithTagsToNAND()\n");
		if(newChunkId >= 0)
		{
