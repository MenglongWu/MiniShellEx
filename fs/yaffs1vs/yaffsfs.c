...
int yaffs_read(int fd, void *buf, unsigned int nbyte)
{
	...
	else if( h && obj)
	{
		...
		if(nbyte > 0)
		{
			nRead = yaffs_ReadDataFromFile(obj,(__u8*)buf,pos,nbyte);
			if(nRead >= 0)
			{
...
int yaffs_write(int fd, const void *buf, unsigned int nbyte)
{
	...
	else if( h && obj)
	{
		...
		nWritten = yaffs_WriteDataToFile(obj,(const __u8*)buf,pos,nbyte);
...
yaffs_off_t yaffs_lseek(int fd, yaffs_off_t offset, int whence) 
{
	yaffsfs_Handle *h = NULL;
	yaffs_Object *obj = NULL;
	int pos = -1;
...
yaffs_off_t yaffs_freespace(const char *path)
{
	yaffs_off_t retVal=-1;
	yaffs_Device *dev=NULL;
	char *dummy;
	...
	if(dsc && dsc->magic == YAFFS_MAGIC)
	{
		...
		if(obj && obj->variantType == YAFFS_OBJECT_TYPE_DIRECTORY)
		{
			...
			if(!inList && entry)
			{
				...
				le = (struct yaffsfs_ObjectListEntry *)YMALLOC(sizeof(struct yaffsfs_ObjectListEntry));
