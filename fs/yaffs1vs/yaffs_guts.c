...
static int yaffs_CreateTnodes(yaffs_Device *dev,int nTnodes)
{
    ...
    newTnodes = (yaffs_Tnode*)YMALLOC(nTnodes * sizeof(yaffs_Tnode));
...
static int yaffs_CreateFreeObjects(yaffs_Device *dev, int nObjects)
{
    ...
    newObjects = (yaffs_Object*)YMALLOC(nObjects * sizeof(yaffs_Object));
	...
	newObjects[nObjects - 1].siblings.next = (struct list_head  *)dev->freeObjects;
	...
	list = (yaffs_ObjectList*)YMALLOC(sizeof(yaffs_ObjectList));
	if(!list)
	{
...
char *yaffs_CloneString(const char *str)
{
	...
	if(str && *str)
	{
		newStr = (char*)YMALLOC(strlen(str) + 1);
		strcpy(newStr,str);
	}
...
static int yaffs_InitialiseBlocks(yaffs_Device *dev,int nBlocks)
{
	...
	dev->blockInfo = (yaffs_BlockInfo*)YMALLOC(nBlocks * sizeof(yaffs_BlockInfo));
	// Set up dynamic blockinfo stuff.
	dev->chunkBitmapStride = (dev->nChunksPerBlock+7)/8;
	dev->chunkBits = (__u8*)YMALLOC(dev->chunkBitmapStride * nBlocks);
...
int yaffs_GutsInitialise(yaffs_Device *dev)
{
	...
	dev->localBuffer = (__u8*)YMALLOC(dev->nBytesPerChunk);
	...
	{ 
		...
		dev->srCache = (yaffs_ChunkCache *)YMALLOC( dev->nShortOpCaches * sizeof(yaffs_ChunkCache));
