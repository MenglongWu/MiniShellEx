...
volatile struct s3c_camif_reg *preg_cam = (struct s3c_camif_reg*)0x4F000000;
			...
			unsigned long WinHorOffset, unsigned long WinVerOffset,  unsigned long CoFrameBuffer, unsigned long PrFrameBuffer)
{
	...
	{
		// LCD 24bpp,CAM 16bpp的情况下一下两者模式显示有问题，颜色均失真
		// LCD 24bpp,CAM 24bpp第一种显示正常，第二种显示效果画面上下跳动，颜色均正常
		// 显示效果1. 测试时能看到屏幕被划分成2个小窗口，LCD下半部分没有视频
	...
	// rCIPRSCCTRL=(1<<31)|(CAM_PVIEW_OUTPUT<<30)|(ScaleUp_H_Pr<<29)|(ScaleUp_V_Pr<<28)|(MainHorRatio<<16)|(MainVerRatio); // 16RGB
	rCIPRSCCTRL=(1<<31)|(CAM_RGB24B<<30)|(ScaleUp_H_Pr<<29)|(ScaleUp_V_Pr<<28)|(MainHorRatio<<16)|(MainVerRatio);		// 24RGB
...
void cam_reg_print()
{
	printk("--------- 2440 camif reg ---------\n");
	printk("%8.8x rCISRCFMT 		%8.8x\n",  &preg_cam->cisrcfmt, preg_cam->cisrcfmt);
	printk("%8.8x rCIWDOFST 		%8.8x\n",  &preg_cam->ciwdofst, preg_cam->ciwdofst);
	printk("%8.8x rCIGCTRL  		%8.8x\n",  &preg_cam->cigctrl, preg_cam->cigctrl);
	printk("%8.8x rCICOYSA1 		%8.8x\n",  &preg_cam->cicoysa1, preg_cam->cicoysa1);
	printk("%8.8x rCICOYSA2 		%8.8x\n",  &preg_cam->cicoysa2, preg_cam->cicoysa2);
	printk("%8.8x rCICOYSA3 		%8.8x\n",  &preg_cam->cicoysa3, preg_cam->cicoysa3);
	printk("%8.8x rCICOYSA4 		%8.8x\n",  &preg_cam->cicoysa4, preg_cam->cicoysa4);
	printk("%8.8x rCICOCBSA1 		%8.8x\n",  &preg_cam->cicocbsa1, preg_cam->cicocbsa1);
	printk("%8.8x rCICOCBSA2 		%8.8x\n",  &preg_cam->cicocbsa2, preg_cam->cicocbsa2);
	printk("%8.8x rCICOCBSA3 		%8.8x\n",  &preg_cam->cicocbsa3, preg_cam->cicocbsa3);
	printk("%8.8x rCICOCBSA4 		%8.8x\n",  &preg_cam->cicocbsa4, preg_cam->cicocbsa4);
	printk("%8.8x rCICOCRSA1 		%8.8x\n",  &preg_cam->cicocrsa1, preg_cam->cicocrsa1);
	printk("%8.8x rCICOCRSA2 		%8.8x\n",  &preg_cam->cicocrsa2, preg_cam->cicocrsa2);
	printk("%8.8x rCICOCRSA3 		%8.8x\n",  &preg_cam->cicocrsa3, preg_cam->cicocrsa3);
	printk("%8.8x rCICOCRSA4 		%8.8x\n",  &preg_cam->cicocrsa4, preg_cam->cicocrsa4);
	printk("%8.8x rCICOTRGFMT 		%8.8x\n",  &preg_cam->cicotrgfmt, preg_cam->cicotrgfmt);
	printk("%8.8x rCICOCTRL 		%8.8x\n",  &preg_cam->cicoctrl, preg_cam->cicoctrl);
	printk("%8.8x rCICOSCPRERATIO 	%8.8x\n",  &preg_cam->cicoscpreratio, preg_cam->cicoscpreratio);
	printk("%8.8x rCICOSCPREDST		%8.8x\n",  &preg_cam->cicoscpredst, preg_cam->cicoscpredst);
	printk("%8.8x rCICOSCCTRL 		%8.8x\n",  &preg_cam->cicoscctrl, preg_cam->cicoscctrl);
	printk("%8.8x rCICOTAREA 		%8.8x\n",  &preg_cam->cicotarea, preg_cam->cicotarea);
	printk("%8.8x rCICOSTATUS 		%8.8x\n",  &preg_cam->cicostatus, preg_cam->cicostatus);
	printk("%8.8x rCIPRCLRSA1 		%8.8x\n",  &preg_cam->ciprclrsa1, preg_cam->ciprclrsa1);
	printk("%8.8x rCIPRCLRSA2 		%8.8x\n",  &preg_cam->ciprclrsa2, preg_cam->ciprclrsa2);
	printk("%8.8x rCIPRCLRSA3 		%8.8x\n",  &preg_cam->ciprclrsa3, preg_cam->ciprclrsa3);
	printk("%8.8x rCIPRCLRSA4 		%8.8x\n",  &preg_cam->ciprclrsa4, preg_cam->ciprclrsa4);
	printk("%8.8x rCIPRTRGFMT 		%8.8x\n",  &preg_cam->ciprtrgfmt, preg_cam->ciprtrgfmt);
	printk("%8.8x rCIPRCTRL 		%8.8x\n",  &preg_cam->ciprctrl, preg_cam->ciprctrl);
	printk("%8.8x rCIPRSCPRERATIO	%8.8x\n",  &preg_cam->ciprscpreratio, preg_cam->ciprscpreratio);
	printk("%8.8x rCIPRSCPREDST		%8.8x\n",  &preg_cam->ciprscpredst, preg_cam->ciprscpredst);
	printk("%8.8x rCIPRSCCTRL 		%8.8x\n",  &preg_cam->ciprscctrl, preg_cam->ciprscctrl);
	printk("%8.8x rCIPRTAREA 		%8.8x\n",  &preg_cam->ciprtarea, preg_cam->ciprtarea);
	printk("%8.8x rCIPRSTATUS 		%8.8x\n",  &preg_cam->ciprstatus, preg_cam->ciprstatus);
	printk("%8.8x rCIIMGCPT 		%8.8x\n\n",  &preg_cam->ciimgcpt, preg_cam->ciimgcpt);
}
