...
void lcd_init()
{
	...
	preg_lcd->lcdcon1 = (4 << 8) | (3 << 5) | (0x9 << 1) | (0 << 0);
	preg_lcd->lcdcon1 =0x36c0479;

	// bit[31:24]	2*DCLK		垂直同步信号后多长时间，多少个无效行同步信号，LCD 手册 Tvb
	// bit[24:14]	272		LCD 手册 480 RGB x 272
	// bit[13:6]	2		垂直同步信号结束前多长时间，多少个无效行同步信号，LCD 手册 Tvf
	// bit[5:0]		10		垂直同步信号脉宽占用几个时钟周期，LCD 手册 Tvp
	preg_lcd->lcdcon2 = (2 << 24) | (271 << 14) | (4 << 6) | (8 << 0);

	// bit[25:19]	2	LCD 手册 Thb
	// bit[18:8]	480-1	LCD 手册 480 RGB x 272  Dot
	// bit[7:0]		2 LCD 手册 Thf
	preg_lcd->lcdcon3 = (10 << 19) | (479 << 8) | (19);

	// bit[7:0]		41		水平同步脉冲宽度，LCD 手册 Thp
	...
	preg_lcd->lcdcon5 = ( 1 << 11) | ( 0 << 10 ) | ( 1 << 9 ) | ( 1 << 8 ) | ( 0 << 7 ) | ( 0 << 6 ) | ( 1 << 3 ) | ( 0 << 1 ) | ( HWSWP ) ;


	// 显存地址
	// preg_lcd->lcdsaddr1 = ( ((unsigned long)embedsky_LCD_BUFFER) >> 1) & ~(3 << 30);
	// preg_lcd->lcdsaddr2 = ( ((unsigned long)(embedsky_LCD_BUFFER + LEN_LCD_BUF) >> 1) + 1) & 0x1fffff;
	// preg_lcd->lcdsaddr3 =  272 * 32/16;// 一行的长度，单位是半字

	preg_lcd->lcdsaddr1 = ( ( ( unsigned long ) embedsky_LCD_BUFFER >> 22 ) << 21 ) | M5D ( ( unsigned long ) embedsky_LCD_BUFFER >> 1 ) ;
	preg_lcd->lcdsaddr2 = M5D( ( ( unsigned long) embedsky_LCD_BUFFER + ( SCR_XSIZE_TFT * LCD_YSIZE_TFT * 2 ) ) >> 1 ) ;
	preg_lcd->lcdsaddr3 = ( ( ( SCR_XSIZE_TFT - LCD_XSIZE_TFT ) / 1 ) << 11 ) | ( LCD_XSIZE_TFT /1 ) ;
	// preg_lcd->lcdsaddr1 = 0x19ea34d8;
	// preg_lcd->lcdsaddr2 = 0xc32d8;
