...
void Wr_CamIIC(unsigned long slvAddr, unsigned long addr, unsigned char data)
{
	_CAMiicMode      = CAMWRDATA;
	_CAMiicPt        = 0;
	_CAMiicData[0]   = (unsigned char)addr;
	_CAMiicData[1]   = data;
	_CAMiicDataCount = 2;
...
void Rd_CamIIC(unsigned long slvAddr,unsigned long addr,unsigned char *data)
{
...
//				rIICCON = 0x6f;			//Resumes IIC operation with NOACK in case of S5X532 Cameara	
		...
		case CAMWRDATA:
			...
			for(i=0;i<10;i++);                  //for setup time until rising edge of IICSCL
				rIICCON &= ~(1<<4);                     //resumes IIC operation.
			break;
		case CAMSETRDADDR: //printk("[S%d]",_iicDataCount);
			if((_CAMiicDataCount--)==0) {
				rIICSTAT = 0xd0;                //stop MasTx condition 
				rIICCON  &= ~(1<<4);                //resumes IIC operation.
...
// 	printk("Input Write Page No of %s\n=>", CAM_NAME);
// 	pageNo = (unsigned char)Uart_GetIntNum();
// #endif

// 	printk("Input Write Register Address of %s\n=>", CAM_NAME);
// 	RegAddr = (unsigned char)Uart_GetIntNum();
    
// 	printk("Input Write Transfer Data into %s\n=>", CAM_NAME);
// 	RegData = (unsigned char)Uart_GetIntNum();

// #if (USED_CAM_TYPE==CAM_S5X532)||(USED_CAM_TYPE==CAM_S5X3A1)
// 	Wr_CamIIC(CAM_ID, (unsigned char)0xec, pageNo);  // set Page no
// #endif
// 	Wr_CamIIC(CAM_ID, (unsigned char)RegAddr, RegData); // set register after setting page number
}
...
// 	printk("Input Read Register Address of %s\n=>", CAM_NAME);
// 	RegAddr = (unsigned char)Uart_GetIntNum();
// 	Rd_CamIIC(CAM_ID, RegAddr, &rdata[0]); 
// 	printk("Register Addr: 0x%2x, data: 0x%2x\n", RegAddr,rdata[0]);
}
...
int CameraModuleSetting(void)
{
	...
	irq_request(ISR_IIC_OFT, Cam_IICInt);
	rINTMSK &= ~(BIT_IIC);
